...
class Canvas(Entity):
    def __init__(self, width=1024, height=1024, color=color.white):
        ...
        self.texture_buffer._texture.setup2dTexture(
            width, height, PandaTexture.TUnsignedByte, PandaTexture.FRgba
        )
        self.texture_buffer._texture.setRamImageAs(self.img.tobytes(), "RGBA")
        self.texture = self.texture_buffer
        self.texture._texture.setRamImageAs(self.img.tobytes(), "RGBA")
